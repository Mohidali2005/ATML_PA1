...
method in task two so each run leaves behind evidence of whether it
trained as intended
"""
...
def save_training_curve(history,name,cfg,alignment_key=None,alignment_label=None):
    ...
    to a csv table and plots the classification loss and the mean
    validation macro f1 for the given run alongside an optional
    alignment metric such as the mmd loss or the domain discriminator
    accuracy
    """
